/*!
	Salsa20 (https://github.com/Kruithne/node-blte)
	Author: Dmitry Chestnykh <dmitry@codingrobots.com>
	Modified: Kruithne <kruithne@gmail.com>
	License: MIT
 */
...
/**
 * Error thrown by the Salsa20 class.
 * @class Salsa20Error
 */
class Salsa20Error extends Error {
	constructor(message, ...args) {
		message = 'Salsa20: ' + util.format(message, args);
		super(message);
...
class Salsa20 {
	 ...
	 */
	constructor(rounds, nonce, key) {
		if (nonce.length !== 8)
			throw new Salsa20Error('Unexpected nonce length. 8 bytes expected, got %d', nonce.length);

		if (key.length !== 16 && key.length !== 32)
			throw new Salsa20Error('Unexpected key length. 16 or 32 bytes expected, got %d', key.length);
	 ...
	 */
	process(buf) {
		let out = new Bufo(buf.byteLength);
		let bytes = this.getBytes(buf.byteLength);
